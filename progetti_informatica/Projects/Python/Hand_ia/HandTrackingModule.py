...
class handDetector():
    ...
    def findHands(self,img,draw = True):
        color1 = self.mpDraw.DrawingSpec(color=(255,255,255), thickness = 2, circle_radius = 4)
        color2 = self.mpDraw.DrawingSpec(color=(200,200,200), thickness = 2)
        imgRGB = cv.cvtColor(img,cv.COLOR_BGR2RGB)
        self.results = self.hands.process(imgRGB)
        if self.results.multi_hand_landmarks:
            for handLM in self.results.multi_hand_landmarks:
                if draw:
                    self.mpDraw.draw_landmarks(img,handLM,self.mpHands.HAND_CONNECTIONS, color1, color2)
